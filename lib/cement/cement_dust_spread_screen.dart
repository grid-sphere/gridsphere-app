...
class _CementDustSpreadScreenState extends State<CementDustSpreadScreen>
  ...
  Future<void> _fetchDataAndCalculate() async {
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        http.get(Uri.parse('$_baseUrl/live-data/${widget.deviceId}'),
            headers: headers),
        // Call 2: Get last 24h history (Daily range)
        http.get(
            Uri.parse(
                '$_baseUrl/devices/${widget.deviceId}/history?range=daily'),
            headers: headers),
      ]);
      ...
      if (liveResponse.statusCode == 200) {
        ...
        if (data != null && data is Map) {
          setState(() {
            ...
            windSpeed =
                double.tryParse(data['wind_speed']?.toString() ?? "0") ?? 0.0;
            humidity =
                double.tryParse(data['humidity']?.toString() ?? "0") ?? 0.0;
            sunlight =
                double.tryParse(data['light_intensity']?.toString() ?? "0") ??
                    0.0;

            // MEMS/Vibration
            memsCurrent =
                double.tryParse(data['vibration']?.toString() ?? "0") ??
                    MockDataStore()
                        .getOrGenerateData(widget.deviceId)['memsCurrent'];
          });
        }
      }
          ...
          setState(() {
            ...
            if (historyList.first.containsKey('vibration')) {
              memsHistory24h = historyList.map<double>((item) {
                return double.tryParse(item['vibration']?.toString() ?? "0") ??
                    0.0;
              }).toList();
            } else {
              // Keep existing mock pattern if DB doesn't have vibration history
              memsHistory24h = MockDataStore()
                  .getOrGenerateData(widget.deviceId)['memsHistory'];
            }
          });
        }
      }
  ...
  void _runAdvancedAlgorithms() {
    ...
    if (airScore <= 25)
      ...
      airRiskCategory = "Manageable";
    else if (airScore <= 70)
      airRiskCategory = "Elevated";
    else if (airScore <= 85)
      airRiskCategory = "High Risk";
    else
      airRiskCategory = "Severe Condition";

    // Exposure
    exposureLoad = pmHistory24h.fold(0.0,
        (sum, pm) => sum + (pm * (pm > 60 ? (pm > 100 ? 2.0 : 1.5) : 1.0)));

    // Momentum
    double pm30minAvg = (pmHistory24h.isNotEmpty)
        ? (pmHistory24h.last + pmHistory24h[max(0, pmHistory24h.length - 2)]) /
            2
        : pm25;
    ...
    if (momentum > 0.15)
      ...
      pollutionMomentum = "Slowly Increasing";
    else if (momentum > -0.05)
      pollutionMomentum = "Stable";
    else
      pollutionMomentum = "Improving Fast";
    ...
    operationalContribution =
        totalSpikes == 0 ? 0 : (linkedSpikes / totalSpikes) * 100;
    ...
    drySurfaceRisk = (humidity < 40 && sunlight > 5000 && windSpeed > 4)
        ? "High Dust Lift Risk"
        : "Stable Conditions";

    if (memsCurrent > 60 && windSpeed > 5)
      activityWindMatrix = "Dangerous: Activity + Dispersion";
    else if (memsCurrent > 60)
      activityWindMatrix = "Localized Activity Spikes";
    else if (windSpeed > 5)
      activityWindMatrix = "Wind-Driven Resuspension";
    else
      activityWindMatrix = "Safe Operating Zone";
  ...
  void _onNavTapped(int index) {
    if (index == 0) {
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) =>
                  CementEmissionScreen(deviceId: widget.deviceId)));
    } else if (index == 4) {
      Navigator.push(
          context,
          MaterialPageRoute(
            ...
            child: _isLoading
                ? Center(
                    child: CircularProgressIndicator(
                        color: AppTheme.primaryColor)) // DYNAMIC SPINNER
                : TabBarView(
                    controller: _tabController,
        ...
        children: [
          ...
          GestureDetector(
            onTap: () => _showDetailBottomSheet(
                context,
                "Environmental Score",
                "A composite score (0-100) combining Pollution (PM/VOC), Weather Trapping (Stability), and Surface Conditions (Dryness).",
                "$airRiskCategory (Score: ${airScore.toStringAsFixed(0)})",
                LucideIcons.gauge,
                scoreColor),
            child: Container(
                ...
                children: [
                  ...
                  Stack(
                    ...
                    children: [
                      ...
                      Column(
                        children: [
                          Text(airScore.toStringAsFixed(0),
                              style: GoogleFonts.inter(
                                  fontSize: 42,
                                  fontWeight: FontWeight.bold,
                                  color: const Color(0xFF1F2937))),
                          Text(airRiskCategory,
                              style: GoogleFonts.inter(
                                  fontSize: 12,
                                  fontWeight: FontWeight.bold,
                                  color: scoreColor)),
          ...
          GridView.count(
            ...
            children: [
              _buildMetricCard(
                  "Exposure Load",
                  exposureLoad.toStringAsFixed(0),
                  "Cumulative",
                  LucideIcons.layers,
                  Colors.purple,
                  "Total environmental burden over 24h weighted by severity."),
              _buildMetricCard(
                  "Momentum",
                  pollutionMomentum,
                  "Trend",
                  ...
                  Colors.blue,
                  "Rate of change in pollution levels vs 30-min average."),
              _buildMetricCard(
                  "Confidence",
                  aqConfidence,
                  "Data Quality",
                  LucideIcons.signal,
                  Colors.green,
                  "Reliability of sensor data based on fluctuation variance."),
              _buildMetricCard(
                  "Sensor Health",
                  sensorHealth,
                  "System",
                  ...
                  Colors.teal,
                  "Operational status of PM and environmental sensors."),
              _buildMetricCard(
                  "PM 2.5 Raw",
                  ...
                  LucideIcons.cloud,
                  Colors.grey,
                  "Direct sensor reading before algorithmic adjustment."),
              _buildMetricCard(
                  "TVOC Level",
                  "${tvoc.toStringAsFixed(0)} ppb",
                  "Gas",
        ...
        children: [
          GestureDetector(
            onTap: () => _showDetailBottomSheet(
                ...
                "Percentage of pollution spikes linked to detected machinery vibration.",
                "${operationalContribution.toStringAsFixed(0)}% Plant vs ${passiveContribution.toStringAsFixed(0)}% Passive",
                LucideIcons.factory,
              ...
              child: Column(
                children: [
                  Row(
                      ...
                      children: [
                        Text("Operational Contribution",
                            style: GoogleFonts.inter(
                                fontSize: 14, fontWeight: FontWeight.bold)),
                        Text("${operationalContribution.toStringAsFixed(0)}%",
                            style: GoogleFonts.inter(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                  ...
                  ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                          value: operationalContribution / 100,
                          minHeight: 12,
          ...
          _buildFullWidthCard(
              "Dust Gen Efficiency",
              dustGenEfficiency.toStringAsFixed(2),
              dustGenEfficiency > 1.5 ? "Inefficient" : "Efficient",
              LucideIcons.settings,
              dustGenEfficiency > 1.5 ? Colors.red : Colors.green,
              "Ratio of PM during High Activity vs Low Activity."),
          const SizedBox(height: 12),
          _buildFullWidthCard(
              "Rain Recovery",
              rainRecoveryTime,
              ...
              Colors.blue,
              "Estimated time for dust levels to return to pre-rain baseline."),
          const SizedBox(height: 12),
          _buildFullWidthCard(
              "Dry Surface Risk",
              drySurfaceRisk,
              "Forecast",
              LucideIcons.sun,
              drySurfaceRisk.contains("High") ? Colors.orange : Colors.blue,
              "Prediction based on Humidity, Sunlight, and Wind Speed."),
          const SizedBox(height: 12),
          _buildFullWidthCard(
              "Activity-Wind Matrix",
              activityWindMatrix,
              "Interaction",
              LucideIcons.wind,
              activityWindMatrix.contains("Dangerous")
                  ? Colors.red
                  : Colors.teal,
              "Cross-analysis of MEMS activity and Wind Speed."),
          const SizedBox(height: 12),
          _buildFullWidthCard(
              "Spike Frequency",
              "$spikeFrequency / 24h",
              ...
              Colors.purple,
              "Number of significant PM spikes recorded in the last 24 hours."),
          const SizedBox(height: 40),
        ],
      ),
    );
